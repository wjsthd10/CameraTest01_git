...
public class DeviceOrientation {
    ...
    public DeviceOrientation(float[] pitches, float[] rolls) {
    ...
    private int calculateOrientation() {
        if (((orientation==ORIENTATION_PORTRAIT || orientation==ORIENTATION_PORTRAIT_REVERSE) && (averageRoll>-30 && averageRoll<30))){
            if (averagePitch>0) return ORIENTATION_PORTRAIT_REVERSE;
            ...
            if (Math.abs(averagePitch)>=30){
                ...
                if (averageRoll>0)return ORIENTATION_LANDSCAPE_REVERSE;
                else return ORIENTATION_LANDSCAPE;
            }
        }
    }
